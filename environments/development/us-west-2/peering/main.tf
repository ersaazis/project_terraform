module "control_to_application" {
  ...
  peering_name = "development-control-to-application-peering"
  ...
  accepter_vpc_cidr         = try(data.terraform_remote_state.application.outputs.vpc_cidr, "10.22.0.0/16")
...
module "application_to_database" {
  ...
  peering_name = "development-application-to-database-peering"

  requester_vpc_id          = try(data.terraform_remote_state.application.outputs.vpc_id, "")
  requester_vpc_cidr        = try(data.terraform_remote_state.application.outputs.vpc_cidr, "10.22.0.0/16")
  requester_route_table_ids = compact(concat(
    try([data.terraform_remote_state.application.outputs.public_route_table_id], []),
    try(data.terraform_remote_state.application.outputs.private_route_table_ids, [])
  ))
  ...
  accepter_vpc_cidr         = try(data.terraform_remote_state.database.outputs.vpc_cidr, "10.32.0.0/16")
  accepter_route_table_ids = compact(concat(
...
module "control_to_database" {
  ...
  peering_name = "development-control-to-database-peering"
